...
function sortAccountsByLastName(accounts) {
  ...
  const results = accounts.sort((act1, act2) => act1.name.last.toLowerCase() < act2.name.last.toLowerCase()? -1 : 1);
  return results;
}
...
function getAccountFullNames(accounts) {
  ...
  const results = accounts.map((act) => `${act.name.first} ${act.name.last}`);
  return results;
}
